...
class simclr(nn.Module):
  ...
  def loss_cal(self, x, x_aug):
    ...
    batch_size, _ = x.size()
    #x_abs = x.norm(dim=1)
    #x_aug_abs = x.norm(dim=1)
    #x_aug_abs = x_aug.norm(dim=1)

    x = F.normalize(x, dim=-1)
    dis = torch.norm(x_aug[:, None] - x_aug[None, :], dim=-1)**2
    loss = torch.log(torch.exp(-dis * 2).mean())
...
def plot_tsne(X, Y):
  ...
  def scatter(x, colors, cls_num=10):
      # We choose a color palette with seaborn.
      palette = np.array(sns.color_palette("hls", cls_num))
  ...
  plt.savefig('tsne-generated-uni.png', dpi=120)
...
if __name__ == '__main__':
    ...
    dataset = TUDataset(path, name=DS, aug=args.aug, noise_rate=args.noise_rate).shuffle()
    ...
    with open('logs_uniform/log_' + args.DS + '_' + args.aug + '_noise' + str(args.noise_rate), 'a+') as f:
